...
export function canavsDistance(p1: IPoint, p2: IPoint) {
  const dx = p2.canvasX - p1.canvasX;
  const dy = p2.canvasY - p1.canvasY;
  return Math.sqrt(dx * dx + dy * dy);
}

export function allowMove(p1: IPoint, p2: IPoint): boolean {
